function fca_eoi_sticky_sidebar(options) {
	...
	jQuery('.fca-eoi-sticky-widget-clone-' + options.sidebar).remove(); // clear fixed mode p1
	...
	for ( var i = 0; i < options.widgets.length; i++ ) {
		...
		if ( widget_obj.attr('id') ) { 
			...
			widgets[i].clone.addClass('fca-eoi-sticky-widget-clone-' + options.sidebar);
