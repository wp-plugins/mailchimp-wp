jQuery( document ).ready(function( $ ){
	...
	// Attach tooltips
	$( '.fca_eoi_form [name=email], .fca_eoi_form [name=name]' ).each( function() {
	...
	$( '.fca_eoi_form [name=email],.fca_eoi_form [name=name]' ).focus( function() {
	...
	$( '.fca_eoi_form' ).submit( function( e ) {
		...
		$.ajax( {
			...
			, data: { 'email': email, 'name': name, 'action': 'fca_eoi_subscribe', 'list_id': list_id }
			, type: 'POST'
			, datatype: 'text'
		} ).done( function( data ) {
